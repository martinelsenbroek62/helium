...
window.keysMap       = {
  ...
  'Food and Waste'                              : 'emissions_food_and_waste_total',
  'Food and Waste per Capita'                   : 'emissions_food_and_waste_total_per_capita',
  ...
  'Other Fuels per Capita'                      : 'emissions_heating_total_per_capita',
...
window.regionalAvgMap = {
  ...
  'Food and Waste Total'                  : 'emissions_food_and_waste_total',
  'Food and Waste Total per Capita'       : 'emissions_food_and_waste_total_per_capita',
...
$.get(location.pathname + '/results.json?histograms=Overall,Home Energy,Transportation,Food and Waste,Overall per Capita,Home Energy per Capita,Transportation per Capita,Food and Waste per Capita,Heat').then(function(resp){
  ...
  if(window.resp = resp){
    ...
    if(resp.data.related_survey_results.length){
        for(row in resp.data.related_survey_results){
            newResultsObj = {}
            results = resp.data.related_survey_results[row].results
            year = results.year
            for(key in results){
                _key = key
                _value = results[key]
                newResultsObj[keysMap[key]] = parseFloat(parseFloat(_value).toFixed(1))
            }
            newResultsObj['year'] = year
            resp.data.historical.push(newResultsObj)
        }
    }

    // Sort by year
    resp.data.historical.sort(function(a,b){
      return a.year > b.year;
    });

     //find users location
     resp.usersLocation = resp.data.questions.filter(function(row){
      if(row.question.match(/what state do you live/i)){
        return true
      }
     })[0].answer

    // resp.usersLocation = 'Vermont';
...
$(document).on('dataLoaded', function(){
  ...
  overallSeries = [
    ...
    {
      name: 'Food and Waste',
      data: fetch('emissions_food_and_waste_total')
    },
    {
      name: 'Offsets',
